    ...
    <el-table :data="rights" border style="width: 100%">
      ...
      <el-table-column label="权限等级">
        <template slot-scope="scope">
          <el-button :type="btntype[scope.row.level]" plain disabled
            >第{{ btntxt[scope.row.level] }}级</el-button
          >
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    getPaginRights() {
      ...
      if (endindex >= this.allrights.length)
        endindex = this.allrights.length - 1;
